...
Engagement workspace for the **Coastline Retail Group Pty Ltd** simulation, the
**Data Cloud Consultant** certification track. Hemayet plays the consultant; the
company and every record in it are fictional. See `README.md` for the brief and scope.

## The org

Target org alias **`coastline`**, a Developer Edition org authorised locally.

```bash
sf org display --target-org coastline
sf data query --target-org coastline --query "SELECT COUNT() FROM Account"
```
...
- **`.mcp.json`** starts the Salesforce DX MCP server pinned to the **`coastline`**
  ...
  `sf org login web --alias coastline` first.
